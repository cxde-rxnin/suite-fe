...
function HotelDetailsPage() {
  ...
  useEffect(() => {
    async function fetchHotel() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/hotels/${objectId}`);
    ...
    async function fetchRooms() {
      ...
      setRoomsError(null);
      try {
        const res = await fetch(`/api/hotels/${objectId}/rooms`);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-900">
      <Header />
      <div className="w-full md:max-w-9xl mx-auto px-2 md:px-5">
        {isLoading ? (
          ...
          <>
            {/* Back Button */}
            <button onClick={() => window.history.back()} className="mt-6 mb-2 px-4 py-2 bg-slate-800 text-white rounded-lg shadow hover:bg-slate-700 transition w-fit flex items-center gap-2">
              ...
              <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black to-transparent px-4 py-3 flex flex-col md:flex-row md:items-end md:justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-white mb-2">
                    <MapPin size={22} />
                    <span className="font-mono text-lg">{hotel.physical_address}</span>
                  </div>
                </div>
              </div>
            </div>
